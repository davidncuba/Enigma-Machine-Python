...
def ConfInicial(opsistema):
    global ringSettings, ringPositions, plugboard, reflector, rotors, ciphertext
    rotors = []
    opcoes = ['I','II','III','IV', 'V']
    rotor = 0
    while rotor < 3:
        numeroRotor = input(f'Escolha o {rotor+1}º Rotor entre  {opcoes}: \n').upper()
        if numeroRotor in opcoes:
            opcoes.remove(numeroRotor)
            rotors.append(numeroRotor)
            rotor += 1 
            clear()
        else:
            print('Rotor inexistente')

    # aneis dos rotores
    ringSettings = input('Digite uma sequencia de 3 letras de A a Z  para os aneis ex: ABC \n').upper()
    clear()
    # posição inicial dos rotores
    ringPositions = input('Digite uma posição inicial dos rotores 3 letras de A a Z ex: ABC \n').upper()
    clear()
    # plug frontral
    ...
    if opsistema == 1:
        plaintext = input("Digite a nova senha\n")
        ciphertext = encode(plaintext)
        senha = CadastrarSenha(ciphertext)
        print('Senha cadastrada')

    elif opsistema == 2:
        plaintext = input("Digite sua senha para logar\n")
        ciphertext = encode(plaintext)
        validacao = VerificarSenha(ciphertext)
        return validacao
    
    elif opsistema == 3:
        plaintext = input("Digite a senha para descriptografar \n")
        ciphertext = encode(plaintext)
        return ciphertext
...
def CadastrarSenha(senha):
    with open('senhas.txt', 'a+') as file:
        file.write(f'{senha};')
    cadastrar = 0
    print('Senha cadastrada com sucesso, \né necessario refazer a configuração e login novamente')

def VerificarSenha(senha):
    with open('senhas.txt', 'r') as file:
        senhas = file.read()
        senhas = senhas.split(';')
        for consulta in senhas:
            if consulta == senha:
                return True
...
clear()
print('#'*50)
print('Bem vindo ao sistema \nPara iniciar é necessario escolher uma das opções abaixo:\n')
print('#'*50)
print('\n')
opcoes = int(input('Para cadastrar senha digite \n[1] para cadastrar senha \n[2] Para Logar   \n[3] Para descriptografar \n'))
clear()
validacao = ConfInicial(opcoes)

if opcoes == 3:
    print(f'A senha descritografada é {validacao}')
    
elif opcoes == 2:
    if validacao:
        print('Login efetuado com sucesso\n')
        print(f'Senha criptografada {ciphertext}')
    else:
        print('senha invalida')
        # ConfInicial(cadastrar)
